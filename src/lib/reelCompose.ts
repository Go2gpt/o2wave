...
export async function componerReel(
  ...
  if (!ffmpegPath) return { error: "ffmpeg no disponible en el servidor." };
  ...
  try {
    ...
    const code = await new Promise<number>((resolve) => {
      ...
      p.on("close", (c) => { if (c !== 0) console.error("ffmpeg reel:", err.slice(-1000)); resolve(c ?? 1); });
    });
    if (code !== 0) return { error: "ffmpeg falló al componer el Reel." };
