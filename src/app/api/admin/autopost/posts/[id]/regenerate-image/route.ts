import { NextResponse } from "next/server";
import { requireAdmin } from "@/lib/adminAudit";
import { regenerarImagenAutopost } from "@/lib/autopost/generator";

export const dynamic = "force-dynamic";
export const runtime = "nodejs";
export const maxDuration = 120;

// Regenera la imagen de una pieza pendiente. Sin body → tipo automático (foto o
// banner según el tipo). Con body {formato:"foto"|"banner", variante:"dark"|"light"}
// el admin elige a mano foto o banner claro/oscuro. Solo admin.
export async function POST(request: Request, { params }: { params: { id: string } }) {
  const auth = await requireAdmin();
  if (!auth.ok) return NextResponse.json({ error: auth.error }, { status: auth.status });

  const body = await request.json().catch(() => ({})) as { formato?: string; variante?: string };
  const formato = body.formato === "banner" || body.formato === "foto" ? body.formato : undefined;
  const variante = body.variante === "light" || body.variante === "dark" ? body.variante : undefined;

  const { data: post } = await auth.admin
    .from("autopost_posts").select("id, cuenta_id, estado, texto, tipo").eq("id", params.id).maybeSingle();
  if (!post) return NextResponse.json({ error: "Pieza no encontrada" }, { status: 404 });
  if (post.estado !== "pending_review") {
    return NextResponse.json({ error: `Solo se puede regenerar una pieza pendiente (estado: ${post.estado}).` }, { status: 409 });
  }

  const r = await regenerarImagenAutopost(auth.admin, post.cuenta_id, post.texto, post.tipo ?? undefined, { formato, variante });
  if ("error" in r) return NextResponse.json({ error: r.error }, { status: 502 });

  const { error } = await auth.admin.from("autopost_posts")
    .update({ imagen_url: r.url, updated_at: new Date().toISOString() }).eq("id", params.id);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });

  return NextResponse.json({ ok: true, imagen_url: r.url });
}
