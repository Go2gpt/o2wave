...
export async function POST(request: NextRequest) {
  try {
    ...
    const { imageUrl, headline, positionX, positionY, fontSize, aspectRatio, textAlign, watermark } = await request.json();
    ...
    const finalBuffer = await composeImage({
      ...
      watermark: watermark !== false, // solo se omite si llega explícitamente false (imagen propia)
    });
